...
#截取版本号
cd /opt/fonsview/NE/fums/deploy/modules/about
version=`cat about.html|grep 'class=\"left-side\">' | cut -d '>' -f 7 | cut -d '<' -f 1| head -1 | tail -1`
ver1=${version:0-8}
ver2=${ver1%%-*}
echo $ver2
...
#0 6 * * * /root/auto_backup_database_round321v1.0.sh
#每个月的第一天 13:10分运行则添加如下一行保存
#10 13 1 * * /root/auto_backup_database_round321v1.0.sh
#显示所有crontab
#crontab -l